...
var calculateTime = function(keyboard, word) {
  ...
  for (var i = 0; i < keyboard.length; i++){ // create an obj of keybord word
      keyBoardObj[keyboard[i]] = i;
  }
  

  for (var i = 0; i < word.length; i++){ // iterate word

      var ch = word[i]
      result += keyBoardObj[ch]

      if(result > index){
      	keyBoardObj[ch] - index
      }else{
      	index - keyBoardObj[ch]
      }
